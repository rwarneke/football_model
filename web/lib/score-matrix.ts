...
export function buildScoreMatrix(params: {
  ...
  const p_x = poissonPmf(lamH, K);
  const p_y = poissonPmf(lamA, K);
  ...
  for (let u = 0; u < K; u += 1) {
    ...
    for (let i = u; i < K; i += 1) {
      ...
      for (let j = u; j < K; j += 1) {
        scoreMatrix[i][j] += pu * px * (p_y[j - u] ?? 0);
      }
    }
  }
