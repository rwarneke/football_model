import type { Metadata } from "next";
import { TeamMatchupPage } from "@/components/team-matchup-page";
import { loadRatings } from "@/lib/ratings";

export const metadata: Metadata = {
  title: "International Football Matchup Predictor",
};

export default async function MatchupPage() {
  const ratings = await loadRatings();
  const lastUpdated = new Date().toLocaleDateString("en-US", {
    month: "long",
    day: "numeric",
    year: "numeric",
  });

  return (
    <main className="px-2 pb-16 pt-8 lg:px-6">
      <div className="flex w-full flex-col gap-10">
        <header className="space-y-4">
          <p className="text-sm uppercase tracking-[0.3em] text-ink-400">
            World Football Ratings
          </p>
          <h1 className="text-3xl font-semibold text-ebony md:text-4xl">
            Matchup Predictor
          </h1>
          <div className="flex items-center gap-4 text-sm text-ink-400">
            <span>Updated {lastUpdated}</span>
          </div>
        </header>

        <TeamMatchupPage
          ratings={ratings}
          winProbabilitiesPath="/model_output/win_probabilities_full.json"
        />
      </div>
    </main>
  );
}
